...
def create_dictionary(list):
    ...
    for e in list:
        if e != '':
            if e.find('is connected to') != -1:
                key = e[:e.find('is connected to')]
                if key not in dict: 
                    dict[key] = None                   
                    values = e[e.rfind('is connected to') + 15: ].split(',')
                    dict[key].append(values)
                                

                                                 
                else:
                    dict[key] = [0].append(e[e.rfind('is connected to'): ])
                    values = e[e.rfind('is connected to'): ].split(',')
                    for v in dict[key]:
                        for i in v[range(0)]:
                            i = i + values
            if e.find('likes to play') != -1:
                key = e[:e.find('likes to play')]
                if key not in dict:
                    dict[key] = [[],[]]
                    dict[key] = [1].append(e[e.rfind('is connected to'):])
                else:
                    dict[key] = [1].append(e[e.rfind('is connected to'):])
    return dict
...
def create_data_structure(string_input):
    ...
    network = create_dictionary(list)
    return network
