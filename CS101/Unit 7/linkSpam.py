...
def is_reciprocal(graph,match,start,k): 
    ...
    if k == 0:        
        if start == match:
            ...
            return False
    if match in graph[start]:
        return True        
    elif k >1: 
        for e in graph[start]:
            if is_reciprocal(graph,match,e,k-1):
                return True
    return False
...
def compute_ranks(graph,k):
    ...
    for i in range(0, numloops):
        ...
        for page in graph:
            newrank = (1 - d) / npages
            for node in graph:
                if page in graph[node] and not is_reciprocal(graph,node,page,k):
