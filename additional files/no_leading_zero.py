...
def compile_formula(formula, verbose=False):
    ...
    # check if the formula has any words with more than one letter
    # if so modify formula so that it includes something like:
    # 'A + B == AB and A != 0
    checkmult = list(re.findall('([A-Z]+)', formula))
    for i in checkmult:
        if len(i) > 1:
            formula = formula + " and {0} != 0".format(i[0])
