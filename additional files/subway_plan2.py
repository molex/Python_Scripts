    ...
    for line, stations in lines.items():
      ...
      for i, station in enumerate(stations):
        if station not in network:
          network[station] = {}
        for neighbor in neighbors(stations, i):
          network[station][neighbor] = line
    return network

def neighbors(stations, i):
  return (stations[j] for j in (i-1, i+1) if len(stations) > j >= 0)
...
def ride(here, there, system=boston):
    ...
    return shortest_path_search(here, system.get, lambda stn: stn == there)


def longest_ride(system):
  stations = system.keys()
  longest = []
  for from_ in stations:
    for to in stations:
      if from_ != to and (not longest or (not (from_ in longest and to in longest))):
        longest = max((longest, ride(from_, to, system)), key=len)
  return longest
